...
class RefactorHandler {
    getAll = <modelType>(model: mongoose.Model<any>, modelName: string) => expressAsyncHandler(async (req: Request, res: Response): Promise<void> => {
        ...
        if (modelName === 'users') {
            const sanitizedUsers = documents.map(user => sanitization.User(user));
    ...
    getOne = <modelType>(model: mongoose.Model<any>) => expressAsyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
